...
The important thing is that we change `USERNAME` in the name of our user. to show what it is, we run `whoami`.

Once we have saved the file, we execute the following command to start NGINX.

If we now access the IP or address of the server via Webborswer and receive the message "Welcome to NGINX", we have configured NGINX correctly.


Next we create the configuration file for our Rails app. but first we delete the existing default ngnix configuration:
...
* server_name
  * Instead of the underscore, you can enter your domain here on which your Rails app should run. With underscore, every domain that is routed to the server is served with the Rails app.
* root
...
* location /cable
  * If you are sure that your app will not use an action cable for websokets, you can remove this block from the file.
...
After creating the project folder, rails new runs bundle install to install the necessary gems and then yarn to install the necessary JS dependencies. We can then switch to the project folder using `cp rails_app` and start development. Happy Coding ;) 

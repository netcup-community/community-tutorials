...
Next to the server, we also need to configure our client.
...
If you are running the client on Windows or macOS, you can use the graphical user interface, to connect to your WireGuard® server. 
The GUI should look like this:
![WireGuard GUI](images/wireguard.png)

To setup the connection to the server, click on the little arrow next to "Add Tunnel" and select "Add empty tunnel":
![WireGuard Add Empty Tunnel](images/wireguard-add-empty-tunnel.png)

After that copy the above configuration into the panel and click "Save":
![WireGuard New Tunnel](images/wireguard-new-tunnel.png)

Now you can connect to the server using the "Activate" button:

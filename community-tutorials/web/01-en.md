...
Now press ctrl + w and enter: `post_max_size = 8M` now replace this line with `post_max_size = 10G` - how big should uploaded files at max be? <br>
Now press ctrl + w and enter: `upload_max_filesize = 2M` now replace this line with `upload_max_filesize = 10G` - how big should uploaded files at max be? <br>
...
First make sure your domain or subdomain is located to your server in its dns setting <br>
...
```
Replace every `DOMAIN` with your Domain and set a folder path under "DocumentRoot", all files in this folder will be aviable on your DOMAIN in the web <br/>
...
```
Save your edit with ctrl + x, then press y and now save it with enter <br/>
Now add the following line `Alias /pma /var/www/pma` in one of your Apache Configfile of the Domain you want with `nano /etc/apache2/sites-enabled/FILE.conf` and add this line to every vhost on port 443 <br>
Now restart Apache2 with `service apache2 restart` <br>
...
```
Change DE if you do not live in germany, look [here](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements) for the ISO code of your land <br/>
Now save your edit with ctrl + x, then press y and now save it with enter <br/>

...
If you now visit your subdomain with a webbrowser, you should see the default text, that a website is under construction. If not, make sure the document root is the one where the default index.html is located. Also doublecheck, that it is the right IP address in the previous step.
...
Try to navigate with a webbrowser to ddns.mydomain.com/.env
You should see the error 403 (forbidden). If you can see the environment file you should immediately update the .htaccess file and add the .env into it:
...
Now navigate with a webbrowser to ddns.mydomain.com/update.php
...
Please create a new file `nano ddns-script.sh` on that deveice and fill in the code:

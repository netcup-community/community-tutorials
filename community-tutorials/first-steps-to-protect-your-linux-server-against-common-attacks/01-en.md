...
4. Next time you login to you server you can use your new user: `<username>@<host>`
...
Now open the file called `authorized_keys`, for example with the command line text editor *nano*:
```bash
...
- With PuTTY first create a new session in the `Session` tab:
  - In the upper field `Host Name` enter `<username>@<host>`
    ...
    `<host>` can be either an ip adress or the domain name of your remote server.

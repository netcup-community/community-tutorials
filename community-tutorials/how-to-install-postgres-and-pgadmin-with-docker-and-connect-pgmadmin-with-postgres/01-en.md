...
docker -v
```
Output should look like this, maybe with another version:
```
...
Create the file docker-compose.yaml file with a texteditor:
...
In this file are two services defined: One for PostgreSQL and one for pgAdmin. Both services have a fix container-name and both have its own volume. In the environments-definition you can see the credentials to access the services. The postgres servuce will be exposed on port 5432 to the outside. pgadmin will be exposed on port 8080 to the outside. Both services are using the same docker network "backend_network", which is also defined in the networks-definition.
...
# Step 4 - Checking status of docker containers
...
# Step 5 - Check logs of the containers
First check the logs of the postgres database container:
```
docker logs db
```

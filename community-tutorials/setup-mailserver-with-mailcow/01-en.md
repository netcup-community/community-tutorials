...
Now we open the DNS management of our domain (in this example we use [Cloudflare](https://www.cloudflare.com/)) and create a new `A` record.  
...
After a few seconds, we can find the Mailcow webinterface at `https://mail.example.com`.  
...
For this we go to `Configuration` => `Server Configuration` => `Configuration` => `ARC/DKIM-Keys`.  

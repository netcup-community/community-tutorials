...
- A own domain _(in our example `example.com`)_

# Step 1 - (Preparation: Packages and Docker)
To start, we update the package lists and install `git` and `curl`:
```
...
_If necessary, we can also repeat this record for our IPv6 address. For this we use the type `AAAA` and our IPv6 address._  
...
and clone the latest version of Mailcow here:
```
...
We can now find the Mailcow webinterface at `https://mail.example.com`.  
To log in we use `admin` _(username)_ and `moohoo` _(password)_.

![Mailcow Login](./images/mailcow-login.png)

# Schritt 4.1 - (Change Mailcow admin password)
...
# Schritt 5 - (Setting up a new domain - Mailcow)
To be able to send and receive emails, we need to add the domain in Mailcow.  
For this we go in the webinterface `Configuration` => `Email Setup` and click `Add domain`:  
...
Here we enter a `username (left part of the e-mail address)` and set a password:  

![Add new mailbox](./images/mailcow-new-mailbox.png)

# Step 5.1 - (Setting up a new domain - DNS)
In order to be able to receive and send emails correctly, we still need to create some DNS records for the domain.  
For this we switch back to the DNS management of our domain.  
...
For this we go under `Configuration` => `Server Configuration` => `Configuration` to `ARC/DKIM-Keys`.  
...
To log in, use your username / email and the password you set.  
...
<!-- TODO: Use email account in a mail client -->

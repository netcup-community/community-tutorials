...
# While Loops
While loops are executing commands as long as the exit code of a certain command is 0. Especially infinite While statements are often needed to execute something periodically. Let's take a look at them:
```sh
while true
do
	sleep 2
...
```
In this case, `true` is again nothing more than a command provided by our shell which always has the exit code 0. While statements often contain sleep statements so they are not executing too fast. Also, we can utilize While statements to execute something for example 5 times:
```sh
i=0

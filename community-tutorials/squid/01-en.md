...
# Requirements
You need a Debian or Ubuntu Server with an Internet Connection, which is accessible from the internet over a static IP on Port 3128. <br>
...
This Configuration allows connecting to the HTTP proxy using a username and a password and makes your origin IP invisible for the Web servers you visit. <br>
...
```
Now save your edit with ctrl + x, then press y and now save it with enter 
...
Now enabled "Use a proxy server" now enter under "Proxy IP address" the IP address or the hostname of your proxy server and under "Port" the port "3128" <br>
...
Now enter under "address" the IP address or the hostname of your proxy server and under "Port" the port "3128" <br>
Now activate the Checkbox "Don't use the proxy server for local (intranet) addresses" <br>
Now you can click the Button "Save" <br>
Now you will sometimes be asked for one of the username and the password of this username, which we created in step 1.2
...
Now enter under "address" or "hostname" the IP address or the hostname of your proxy server and under "Port" the port "3128" <br>
Now you can click the Button "Save" <br>
Now you will sometimes be asked for one of the username and the password of this username, which we created in step 1.2
...
Now enter under "server" the IP address or the hostname of your proxy server and under "Port" the port "3128" <br>
...
Now enter your IP address or the hostname of your proxy server and the port "3128" <br>
Now click "Ok" and then "apply" <br>
Now if you asked for it (you may need to reboot), enter your username and the password of this username, which we created in step 1.2
...
```
Now add there the following lines and replace "username" with a username and "password" with the password of this username, from one of th users, which we created in step 1.2 and replace "hostname" with the IP address or the hostname of your proxy server and  "port" with the port "3128":
```sh

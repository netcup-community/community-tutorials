...
If you are doing this directly on a domain and NOT on a subdomain or you want to make the site aviable on more sites, I recommend adding a Server Alias to the Configuration like you see below. (If you want to make this site aviable on every Subdomain which loks the following `abc` you can also add as Server Alias `abc.*`.)

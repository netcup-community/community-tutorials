...
sudo docker --version
```
Output should look like this, maybe with a different version or buildnumber:
```
...
# Step 4 - Create a docker-compose.yaml file, with MongoDB and Mongo Express
Create a docker-compose.yaml file with a texteditor:
```
...
Check the status of the docker containers with the command:
```
docker ps -a
```
